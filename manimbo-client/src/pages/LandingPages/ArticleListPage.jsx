...
const normalize = (article) => ({
  _id: article._id || article.id || article.name,
  name: article.name,
  title: article.title,
...
const ArticlesListPage = () => {
  const [articles, setArticles] = useState(seedArticles.map(normalize));
    ...
    fetchArticles()
      .then(({ data }) => {
        ...
        const active = list
          ...
          .map(normalize);
        if (active.length) setArticles(active);
      })
      .catch((err) => {
        if (cancelled) return;
        setError(
          err.response?.data?.message ||
            err.message ||
            'Showing local sample articles — the server is unreachable.'
        );
      });
    return () => {
      cancelled = true;
  ...
  return (
    <div className="flex w-full flex-col gap-6 px-3 pt-3 sm:px-4">
      ...
      <section className="rounded-[2rem] border border-[#efe3d7] bg-[#fdfbf8] px-6 py-10 sm:px-8 lg:px-10">
        ...
        {error ? (
          <p className="mb-4 rounded-[1rem] border border-[#eadbca] bg-[#fff7ee] px-4 py-3 text-sm text-[#6e5b4d]">
            {error}
          </p>
        ) : null}
